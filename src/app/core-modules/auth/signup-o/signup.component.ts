...
const customPasswordValidator: ValidatorFn = (
  ...
  if (!/^[A-Z]/.test(password)) {
    errors.startsWithUppercase = true;
  }

  if (password.length < 6) {
    errors.isAtLeast6Characters = true;
  }

  if (!/[!@#$%^&*(),.?":{}|<>]/.test(password)) {
    errors.includesSpecialCharacter = true;
  }

  if (!/\d/.test(password)) {
    errors.includesNumber = true;
  }
...
export class SignupComponent implements OnInit {
  ...
  ngOnInit(): void {
    this.loadForm();
    this.formGroup.get('mobile')?.valueChanges.subscribe((res) => {
      console.log(res);
    });
    ...
    this.userInfoForm = this.fb.group(
      {
        ...
        email: [`user.${this.formGroup.get('mobile')?.value}@soowgood.com`],
        password: [
          this.formGroup.get('userTypeName')?.value == 'Patient'
            ? 'Coppa@123'
            : '',
          [Validators.required, customPasswordValidator],
        ],
  ...
  isValidationCheck(input: string): boolean {
    ...
    // Check for at least one number (any number between 1 and 10)
    const hasNumber1to10 = /[1-9]|10/.test(input); // Matches any number from 1-9 and 10

    // Return true if all conditions are met
    return hasSpecialChar && hasCapitalLetter && hasNumber1to10;
  }
  loadAuth() {
  ...
  sendOtp() {
    ...
    let otpData: SendOtpModel = {
      userName: mobileNo,
      type: 1,
      roleId: this.formGroup.get('userTypeName')?.value,
    };
    this.isLoading = true;

    if (this.formGroup.get('userTypeName')?.value == 'Patient') {
      this.subs.sink = this.UserManageAccountsService.sendOtpByRequest(
        otpData
      ).subscribe({
        next: (res) => {
          if (res.is_success) {
  ...
  verify() {
    ...
    let otp = this.otp;
    this.userInfoModal = false;
    const requestOtp: OtpRequestDto = {
      otp: otp || 0,
      mobileNo: this.formGroup.get('mobile')?.value,
    };

    if (otp) {
      this.subs.sink = this.UserManageAccountsService.verifyOtpByRequest(
        requestOtp
      ).subscribe({
        next: (res) => {
          if (res.is_success) {
            this.userInfoModal = res.is_success;
          } else {
  ...
  // updated start
  async sendUserInfo() {
    const { userTypeName } = this.formGroup.value;
    this.formSubmitted = true;

    if (userTypeName == 'Doctor' && this.userInfoForm?.invalid) {
      this.userInfoForm?.markAllAsTouched();
      console.log(this.userInfoForm);

      return;
    }
    if (userTypeName == 'Patient' && this.patientForm.invalid) {
      ...
      const requestSignup = {
        ...
        name:
          this.userType == 'Patient'
            ? this.patientForm.get('fullName')?.value
            : this.userInfoForm?.value.fullName,
        surname: 'default',
        email:
          this.userType == 'Patient'
            ? this.mobile + '@sg.com'
      ...
      if (this.stepBack1 == false) {
        this.UserManageAccountsService.signupUserByRequest(
          ...
          next: (res) => {
            ...
            if (!res.is_success) {
              this.errorMessage = 'Faild to signup!';
